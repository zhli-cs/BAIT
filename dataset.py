...
class DatasetGenerator():
    def __init__(self, args, train_batch_size=128, eval_batch_size=256, num_of_workers=4,
                 ...
                 use_cutout=None, use_cutmix=False, use_mixup=False, use_generator=False):
        ...
        if train_data_type == 'CIFAR10':
            ...
            num_of_classes = 10
            train_data_path = os.path.join(train_data_path, 'SVHN')
        ...
        if test_data_type == 'CIFAR10':
            ...
            test_dataset = PoisonCIFAR10(args=args, root=test_data_path, transform=test_transform,
                                          poison_rate=poison_rate, perturb_type=perturb_type,
                                          patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                          perturb_tensor_filepath=perturb_tensor_filepath,
                                          add_uniform_noise=add_uniform_noise,
            ...
            test_dataset = PoisonCIFAR100(args=args, root=test_data_path, transform=test_transform,
                                           poison_rate=poison_rate, perturb_type=perturb_type,
                                           patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                           perturb_tensor_filepath=perturb_tensor_filepath,
                                           add_uniform_noise=add_uniform_noise,
            ...
            test_data_path = os.path.join(test_data_path, 'SVHN')
            test_dataset = datasets.SVHN(root=test_data_path, split='test',
                                         download=True, transform=test_transform)
        elif test_data_type == 'PoisonSVHN':
            test_data_path = os.path.join(test_data_path, 'SVHN')
            test_dataset = PoisonSVHN(args=args, root=test_data_path, split='test', download=True, transform=test_transform,
                                       poison_rate=poison_rate, perturb_type=perturb_type,
                                       patch_location=patch_location, seed=seed, img_denoise=img_denoise,
                                       perturb_tensor_filepath=perturb_tensor_filepath,
                                       add_uniform_noise=add_uniform_noise,
...
class PoisonCIFAR10(datasets.CIFAR10):
    def __init__(self, args, root, train=True, transform=None, target_transform=None,
        ...
        if use_generator:
            ...
            checkpoints = torch.load(args.generator_filepath, map_location=device)
            self.generator.load_state_dict(checkpoints)
            ...
            for idx in self.poison_samples_idx:
                ...
                if perturb_type == 'classwise':
                    # Class Wise Poison
                    noise = self.generator(data_input)  
                    noise = noise.squeeze(0).mul(255).clamp_(-255, 255).permute(1, 2, 0).detach().cpu().numpy() 
            ...
            if perturb_type == 'classwise':
                ...
                for class_id in range(len(self.classes)):
                    if class_noises[class_id]:  
                        mean_class_noises[class_id] = np.mean(class_noises[class_id], axis=0) * 8/255
            ...
            for idx in self.poison_samples_idx:
                ...
                if perturb_type == 'classwise':
                    ...
                    noise = patch_noise_extend_to_img(noise, noise.shape, patch_location=self.patch_location)
                if add_uniform_noise: 
                    noise += np.random.uniform(0, 8, (32, 32, 3))
        ...
        if use_generator:
            ...
            for idx in self.poison_samples_idx:
                ...
                if perturb_type == 'classwise':
                    # Class Wise Poison
                    noise = self.generator(data_input)  
                    noise = noise.squeeze(0).mul(255).clamp_(-255, 255).detach().cpu().numpy()  
                    if add_uniform_noise:
                        noise += np.random.uniform(0, 8, (32, 32, 3))
